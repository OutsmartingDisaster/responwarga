...
export default function UsersTable() {
  const supabase = createClient(); // Use client for non-admin operations if needed
  ...
  const fetchUsers = async () => {
    try {
      ...
      // The API route should return a combined list of users with profile data
        ...
        fetchedUsers.map(async (user: User) => {
          ...
          if (userRole === 'org_admin' && user.organization_id) {
            ...
            try {
              ...
              if (assignmentError) console.error(`UsersTable: Error fetching assignments for org ${user.organization_id}:`, assignmentError);
              else {
            ...
            try {
              ...
              if (disasterError) console.error(`UsersTable: Error fetching disaster responses for org ${user.organization_id}:`, disasterError);
              else {
              ...
              <td className="px-4 py-3">
                {editingUser?.id === user.id ? (
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                    (user.profile_role || user.user_metadata?.role) === 'admin' ? 'bg-red-900/50 text-red-200' :
